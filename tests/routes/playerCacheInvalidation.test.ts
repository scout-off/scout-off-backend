...
import { getDriver, upsertPlayer, updatePlayerProgress, getPlayerById } from '../../src/db';
...
describe('Player Cache Invalidation', () => {
  ...
  beforeEach(async () => {
    ...
    await getDriver().run('DELETE FROM players WHERE player_id = ?', [PLAYER_ID]);
  ...
  describe('API returns updated data after cache invalidation', () => {
    it('GET /api/players/:id returns updated tier after cache invalidation', async () => {
      ...
      const dbRow = await getPlayerById(PLAYER_ID);
